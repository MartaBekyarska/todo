...
class TodoApplication (val todoService: TodoService) {
	@GetMapping("/v1/todos")
	fun todos(): MutableList<ToDo> = todoService.getTodos()

	@GetMapping("/v1/todo/{id}")
	fun todo(@PathVariable id: String): ToDo? = todoService.getTodo(id)

	@PostMapping("/v1/todo")
	fun createTodo(@RequestBody toDoRequest: ToDoRequest): ToDo = todoService.createTodo(toDoRequest)
	...
	fun updateTodo(@PathVariable id: String, @RequestBody toDoRequest: ToDoRequest): ToDo? {
		...
		return todo?.id?.let { todoService.update(todo, toDoRequest) }
	}
